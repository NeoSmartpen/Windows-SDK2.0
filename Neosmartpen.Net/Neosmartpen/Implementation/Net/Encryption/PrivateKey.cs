...
namespace Neosmartpen.Net.Encryption
{
    ...
    public class PrivateKey
	{
		...
		public IBuffer PrivateExponent { get; private set; }

		public PrivateKey(IBuffer key)
		{
			ParseBuffer(key);
		}
		...
		private void ParseBuffer(IBuffer key)
		{
			...
			ParseVersion(dataReader);

			ParseModulus(dataReader);

			ParsePublicExponent(dataReader);

			ParsePrivateExponent(dataReader);
		}

		private void ParseVersion(DataReader reader)
		{
			byte tag = reader.ReadByte();
			if (tag == 0x02)
			{
				var temp = ReadInteger(reader);
				...
				Version = (int)ByteArrayToInt(bytes);
			}
			else
		...
		private void ParsePrivateExponent(DataReader reader)
		{
			byte tag = reader.ReadByte();
			if (tag == 0x02)
				PrivateExponent = ReadInteger(reader);
			else
				throw new Exception("Invalid Tag In PublicExponent");
